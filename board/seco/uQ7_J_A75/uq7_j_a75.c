...
#include "pin_muxing_uQ7_J_A75.h"
...
#define USDHC4_CD_GPIO	                          IMX_GPIO_NR(7, 0)
...
struct usdhc_l usdhc_list_spl[CONFIG_SYS_FSL_USDHC_NUM] = {
	{ usdhc3_pads, ARRAY_SIZE(usdhc4_pads), -1, -1 },
	{ usdhc4_pads, ARRAY_SIZE(usdhc3_pads), USDHC4_CD_GPIO, USDHC4_PWR_GPIO },
};
...
static void spl_dram_init( void ) {
	...
	if ( is_mx6dq( ) ) {
		/* QUAD cofiguration doesn't present */
	} else if ( is_mx6dl( ) ) {
...
{
	if (is_mx6dq()) {
		if (!strcmp(name, "imx6q-seco_uQ7_J_A75"))
			return 0;
	} else if ( is_mx6dl( ) || is_mx6solo( ) ) {
		if (!strcmp(name, "imx6dl-seco_uQ7_J_A75"))
			return 0;
	}
