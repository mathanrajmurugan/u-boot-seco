...
#define BOOT_VALIDATE_GPIO                       IMX_GPIO_NR(2, 13)
 ...
 * 	USDHC3  -->  internaluSD   -->  FSL_SDHC: 1
 * 	USDHC1  -->  external uSD  -->  FSL_SDHC: 2
 */
...
struct usdhc_l usdhc_list_spl[CONFIG_SYS_FSL_USDHC_NUM] = {
	{usdhc3_pads, ARRAY_SIZE(usdhc4_pads), -1},
	{usdhc1_pads, ARRAY_SIZE(usdhc1_pads), USDHC1_CD_GPIO},
	{usdhc4_pads, ARRAY_SIZE(usdhc3_pads), USDHC4_CD_GPIO},
};
...
#else
char *board_name = "Seco Q7-i.MX6 - 928";
#endif
...
{
	if (is_mx6dq()) {
		if (!strcmp(name, "imx6q-seco_quadmo747_928"))
			return 0;
	} else if ( is_mx6dl( ) || is_mx6solo( ) ) {
		if (!strcmp(name, "imx6dl-seco_quadmo747_928"))
			return 0;
	}
