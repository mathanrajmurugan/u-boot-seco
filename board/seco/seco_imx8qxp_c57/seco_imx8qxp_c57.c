...
int board_early_init_f(void)
{
	...
	ret = sc_pm_set_clock_rate(-1, SC_R_UART_0, SC_PM_CLK_PER, &rate);
	if (ret != SC_ERR_NONE)
		return 0;
	...
	/* Set UART2 clock root to 80 MHz */
	rate = 80000000;
	ret = sc_pm_set_clock_rate(-1, SC_R_UART_2, SC_PM_CLK_PER, &rate);
	if (ret)
		return ret;

	/* Enable UART2 clock root */
	ret = sc_pm_clock_enable(-1, SC_R_UART_2, SC_PM_CLK_PER, true, false);
	if (ret)
		return ret;
...
#define USB_HUB_RST_GPIO IMX_GPIO_NR(4, 3)
...
void hub_rst(void){
	...
	gpio_request(USB_HUB_RST_GPIO, "rst_hub_gpio");
	gpio_direction_output(USB_HUB_RST_GPIO, 0);
